resource "google_compute_region_network_endpoint_group" "neg_api1" {
  name                  = "ten-wimbledon-p-neg-euwe2a-api1"
...
resource "google_compute_region_network_endpoint_group" "neg_api2" {
  name                  = "ten-wimbledon-p-neg-euwe2a-api2"
  network_endpoint_type = "SERVERLESS"
  region                = "europe-west2"
  cloud_run {
...
resource "google_compute_region_network_endpoint_group" "neg_web" {
  name                  = "ten-wimbledon-p-neg-euwe2a-web"
  network_endpoint_type = "SERVERLESS"
  region                = "europe-west2"
  cloud_run {
...
module "alb_ingress" {
  ...
  version        = "~> 9.0"
  name           = "ten-wimbledon-p-alb-euwe2a-ingress"
...
resource "google_compute_url_map" "routing_map" {
  name            = "ten-wimbledon-p-alb-euwe2a-map"
